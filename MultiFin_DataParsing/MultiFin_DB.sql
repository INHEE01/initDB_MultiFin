...
CREATE TABLE ACCOUNT (
    ...
    MNO INT, -- 회원 고유번호
    TOTALASSET BIGINT DEFAULT 10000000, -- 보유 자산
	WON BIGINT DEFAULT 10000000, -- 보유 중인 한화
	TOTALPROFIT BIGINT DEFAULT 0, -- 평가 손익
    
	FOREIGN KEY ACCOUNT(MNO) REFERENCES MEMBER(MNO) ON UPDATE CASCADE ON DELETE CASCADE
);
INSERT INTO ACCOUNT(ANUM, ACCOUNT, MNO, TOTALASSET, WON, TOTALPROFIT) VALUES(0, '110-100-10000-1', '2', DEFAULT,  DEFAULT,  DEFAULT);
INSERT INTO ACCOUNT(ANUM, ACCOUNT, MNO, TOTALASSET, WON, TOTALPROFIT) VALUES(0, '110-100-10000-3', '3', DEFAULT,  DEFAULT,  DEFAULT);

SELECT * FROM ACCOUNT;
-- -----------------------
-- 모의 투자에서 매수 및 매매한 주식 정보
-- -----------------------
DROP TABLE INVESTEDSTOCK;
CREATE TABLE INVESTEDSTOCK (
	ORDERNUM INT PRIMARY KEY AUTO_INCREMENT, -- 주문번호
	MNO INT, -- 멤버 일련번호
	SNO INT, -- 주식 일련번호
	ANUM INT, -- 계좌 일련번호
	orderdt DATETIME DEFAULT CURRENT_TIMESTAMP, -- 처리 날짜
	stockcode INT, -- 종목코드
    ...
    CONSTRAINT FK_IS_ANUM FOREIGN KEY INVESTEDSTOCK(ANUM) REFERENCES ACCOUNT(ANUM) ON UPDATE CASCADE ON DELETE CASCADE 
);

COMMIT;

INSERT INTO INVESTEDSTOCK(ORDERNUM, MNO, SNO, ANUM, orderdt, stockcode, stockNUM, tradestat, price, cnt, totalPrice) 
VALUES(0, 2, 43372, 1, DEFAULT, 5930, '삼성전자', 'B',  63900, 20, (63900*20));
INSERT INTO INVESTEDSTOCK(ORDERNUM, MNO, SNO, ANUM, orderdt, stockcode, stockNUM, tradestat, price, cnt, totalPrice) 
VALUES(0, 2, 43372, 1, DEFAULT, 5930, '삼성전자', 'B',  63900, 40, (63900*20));
INSERT INTO INVESTEDSTOCK(ORDERNUM, MNO, SNO, ANUM, orderdt, stockcode, stockNUM, tradestat, price, cnt, totalPrice) 
VALUES(0, 2, 43372, 1, DEFAULT, 5930, '삼성전자', 'B',  63900, 30, (63900*20));
INSERT INTO INVESTEDSTOCK(ORDERNUM, MNO, SNO, ANUM, orderdt, stockcode, stockNUM, tradestat, price, cnt, totalPrice) 
VALUES(0, 3, 43372, 2, DEFAULT, 5930, '삼성전자', 'B',  63900, 40, (63900*40));
INSERT INTO INVESTEDSTOCK(ORDERNUM, MNO, SNO, ANUM, orderdt, stockcode, stockNUM, tradestat, price, cnt, totalPrice) 
VALUES(0, 3, 43372, 2, DEFAULT, 5930, '삼성전자', 'B',  63900, 80, (63900*80));
INSERT INTO INVESTEDSTOCK(ORDERNUM, MNO, SNO, ANUM, orderdt, stockcode, stockNUM, tradestat, price, cnt, totalPrice) 
VALUES(0, 3, 43372, 2, DEFAULT, 5930, '삼성전자', 'B',  63900, 120, (63900*120));

SELECT * FROM INVESTEDSTOCK;
SELECT * FROM STOCKPRICE WHERE SRTNCD='5930'; 
SELECT * FROM MEMBER;
SELECT I.ORDERNUM, M.ID FROM INVESTEDSTOCK I, MEMBER M WHERE M.MNO=I.MNO;
